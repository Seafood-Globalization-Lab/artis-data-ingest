...
clean_prod <- function(a_prod_file) {
  # code is copied and changed from `classify_prod_dat.R` function in artis-model repo
      prod_ts <- a_prod_file %>%
        # select columns we are interested in for the purposes of comparing version changes
        select(country, prod_method, species_name_en, species_scientific_name,
               country_iso3_code, species_major_group) %>% 
...
# clean prod versions 
prod_old <- clean_prod(prod_old)
prod_new <- clean_prod(prod_new)
